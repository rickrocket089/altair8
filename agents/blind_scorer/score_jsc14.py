"""Blind Scorer, Sprint 14 second separate pass: JSC reasoning-quality
classification on all 120 BASELINE + ORDER-BEFORE outputs. Design REUSED
UNCHANGED from Sprint 13 per protocol Section 6.

Sequencing enforced, not just documented (Sprint 13's contamination-
prevention discipline, carried forward): refuses to run until the primary
FVBS pass (score_fvbs.py) is locked and stored. Does not read or
reference the FVBS verdicts at all -- re-reads the same anonymized
outputs fresh.
"""
import json
import os
...
RUBRIC = """RUBRIC -- JSC reasoning-quality classification (unchanged from Sprint 13).
...
Alternatives must be named explicitly.
...
If a justification mixes both, classify by the dominant structure. If
genuinely ambiguous between A and B, use C.
"""
...
def run() -> None:
    ...
    if not db.get_memory("blind_scorer", "sprint14_fvbs_scores"):
        raise RuntimeError(
            "Primary FVBS pass (score_fvbs.py) must run and be stored "
            "before JSC scoring -- contamination-prevention sequencing, "
            "not optional."
        )
    ...
    db.set_memory("blind_scorer", "sprint14_jsc_scores", json.dumps(all_scores, indent=2, ensure_ascii=False))
    counts = {}
    for s in all_scores:
        counts[s["jsc"]] = counts.get(s["jsc"], 0) + 1
    print(f"\n[{NAME}] TOTAL: {len(all_scores)} scored. Breakdown: {counts}")
    print("--- stored as blind_scorer/sprint14_jsc_scores ---")
